...
@Component({
  selector: 'add-hero-slides-modal',
  templateUrl: 'add-hero-slides-modal.html',
  styleUrls: ['./add-hero-slides-modal.scss'],
})
export class AddHeroSlidesModalComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private dialogRef: MatDialogRef<AddHeroSlidesModalComponent>,
  ...
  getImageSrc() {
    if (this.category.image) {
      this.imageSrc = this.helperService.getImageUrl(this.category.image, 'gallery', 'thumb');
    }
  }
  ...
  addCategory() {
    ...
    const subscription = this.homeService.addHeroSlide(this.listingCategoryForm.value).subscribe(
      (result: any) => {
        this.spinnerService.hide();

        this.snackbar.openSnackBar('Image added');
  ...
  editCategory() {
    this.spinnerService.show();
    const subscription = this.homeService.updateHeroSlide(this.category.id, this.listingCategoryForm.value).subscribe(
      (result: any) => {
        this.spinnerService.hide();

        this.snackbar.openSnackBar('Slider updated');
        this.dialogRef.close(true);
      },
      (error) => {
        this.spinnerService.hide();
  ...
  onAttachmentChange(event: any) {
    ...
    if (event.target.files && event.target.files.length) {
      ...
      this.uploadService.uploadImage(fd, 'gallery').subscribe((result: HttpEvent<any>) => {
        switch (result.type) {
