...
@Component({
  selector: 'app-forum-user-replies',
  templateUrl: './user-replies.component.html',
  styleUrls: ['./user-replies.component.scss'],
})
export class ForumUserRepliesComponent implements OnInit {
  siteUrl: string;
  ...
  getReplies(page: number = 1) {
    ...
    const subsTopics = this.forumService.getUserReplies(this.queryParams).subscribe(
      (res: any) => {
        this.spinnerService.hide();
